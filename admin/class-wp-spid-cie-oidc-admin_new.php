...
class WP_SPID_CIE_OIDC_Admin {
    ...
    public function handle_key_generation() {
        if ( isset( $_GET['action'], $_GET['_wpnonce'] ) && $_GET['action'] === 'generate_oidc_keys' && wp_verify_nonce( $_GET['_wpnonce'], 'generate_oidc_keys_nonce' ) ) {
            ...
            $jwkPublicKeyString = $publicKey->toString('JWK');
            $jwkPublicKeyArray = json_decode($jwkPublicKeyString, true);
            
            if (is_array($jwkPublicKeyArray)) {
    ...
    public function print_keys_section_info() { 
        print 'Le chiavi crittografiche sono necessarie per garantire la comunicazione sicura con gli Identity Provider. Se non hai ancora generato le chiavi, clicca sul pulsante qui sotto.'; 
    }
	
	public function print_spid_section_info() {
        print 'Inserisci le credenziali fornite da AgID per la federazione OIDC SPID.';
    }

    public function print_cie_section_info() {
        print 'Inserisci le credenziali fornite dal Ministero dell\'Interno per la federazione OIDC CIE.';
    }
